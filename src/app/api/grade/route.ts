...
export async function POST(req: Request) {
    try {
        ...
        const { questionText, studentAnswer, rubric, indicator, cognitiveLevel, customApiKey, customModel } = body;

        const apiKey = customApiKey || process.env.NEXT_PUBLIC_GEMINI_API_KEY;
        const model = customModel || "gemini-2.0-flash";

        if (!apiKey) {
            return NextResponse.json({ error: "API Key tidak ditemukan. Silakan masukkan di menu Pengaturan (Admin)." }, { status: 400 });
        }
        ...
        // Using dynamic model from settings or fallback
